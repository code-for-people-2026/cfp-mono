...
- 管理态按“启用中、已停用”分组，集中提供新增、编辑、停用/恢复和批量导入入口。
...
- 菜单页通过纯 `menuWeek` 模型派生上海业务日期、五日工作周、所选日午晚餐位置、完成度、预订信号和下一截止点；进入时自动请求周一至周五，切日不重新请求。
- 周加载使用单调 revision，只允许当前工作周的最后请求写回；后台刷新保留现有快照，失败时继续展示旧数据。H5 运行期与小程序页面栈保留所选周/日期，从预订配置返回后重新加载该周。
- 页面为最近一个尚未截止的 `open` 餐次调度计时器，到点只推进本地时钟并重新派生状态；切周、数据变化或卸载时重建或清理计时器。
- 自动周界面已经承接生成、覆盖确认、换菜与预订配置衔接；旧日期范围和独立预订批次兼容入口均已移除。
- 草稿餐次的“换一道”先展示当前五道快照，只发送被选中菜品的 `offeringId`，不在前端伪造替代候选；成功响应按餐次稳定标识合并，无候选则保留原快照并引导菜品库。
- 换菜使用目标工作周、目标餐次 revision 和共享 view revision：同餐次生成/换菜互斥，不同餐次可并行合并；切周后的旧响应和写入前发出的旧刷新均不能回滚当前视图。换菜 pending 只锁定目标餐次，页面返回刷新延后到全部在途 mutation 完成。
...
- 配置页返回时，小程序按页面栈唤醒原菜单实例；若配置页是冷启动或直接路由形成的栈根，则以 `redirectTo` 兜底。H5 同样以 `redirectTo` 替换配置路由并通过模块内视图快照恢复工作周和所选日期；两端都会重新读取真实餐次。配置页的自动与手动加载使用单调 revision，只允许最后一次请求更新视图。底部 CTA 继续使用“缺失目标 → 草稿配置/开放 → 查看预订与分享”的纯逻辑优先级。
