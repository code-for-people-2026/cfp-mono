...
> 定位描述沿用"社区私房菜助手"。
> 状态：草稿 v1.13 ｜ 最近更新：2026-07-16 ｜ 负责人：码成工小队
> v1.13 变更：**首轮人工试用前语义校准**——到账只表示桃子手工记录订单是否已收到，不接支付、不增加“已核对”用户状态；聊天默认展示最近消息并向上游标分页，按容量有界留存而非固定两天清理；顾客纠错和数据导出/删除拆为试用后分别评估的 backlog，不再作为部署前默认阻塞项。
> v1.12 变更：**生产接龙与补单语义定稿**——完整接龙按标题日期/餐次覆盖范围内全部 active 订单，不区分此前录入方式；自然语言补单区分追加份数与设置总数；两者都先展示系统理解和最终结果，再由桃子确认。
...
- **MVP（M1）**：记单（接龙粘贴导入 + 语音补私聊 + AI 结构化）、AI 菜单生成、按地址相似度排序的送餐清单（勾选 + 手工到账记录）。
  - **MVP 边界（重要）**：全程**只有桃子一个使用者**，**不含任何顾客侧界面、不做顾客点单、不接任何支付**。顾客仍走原微信群，桃子把信息录进来；系统只让她手工标记某订单“未标记到账 / 已标记到账”，不发起、确认或处理任何资金交易。顾客侧一切（含只读看菜单）均属 M3/V1。
...
**心智模型**：后端是一台"引擎"= 数据 + 操作（记单、改单、出菜单、换菜、标已送、手工记录已到账、查状态/历史…）。
...
| 形态 | NL → 自然语言 / 固定展示卡 / 固定确认卡 | 看明细 + 常用快捷操作（一键换菜、标已送、记录已到账） |
...
- 如果用户问的是**数据模型明确的展示类问题**（如"今天订单情况""今天菜单是什么""谁没送""哪些订单还没标记到账"），agent 调确定性查询工具，返回固定结构卡片；卡片只展示必要数据和少量 CTA。
- 如果用户说的是**数据操作类问题**（如新增/修改/删除订单、改菜单、标送达、记录已到账），agent 先解析出目标资源、动作和字段，返回固定确认卡，明确"将修改哪些资源、改前/改后是什么"；用户确认后才写入。
...
| 类别 | 例子 |
|---|---|
| 汇总 / 状态 | 今天订单情况 · 今天菜单是什么 · 谁没送 · 哪些订单还没标记到账 |
| 记单 | 粘接龙记单 · "7 月 3 日晚餐，加王阿姨 2 份" |
| 改单 / 删单 | "7 月 3 日午餐，王阿姨改 1 份" · "取消明天晚餐李叔那单" |
| 菜单 | 明天午餐随机换一道 · 把番茄牛腩换成香菇滑鸡 · 发群通知 |
| 送餐 / 到账记录 | "26B 送了" · "李叔已付"（这里只记录用户判断；信息不足则先确认具体订单） |
| 历史 | 昨天订单 · 上周三做了什么 · 张阿姨上次点啥 |
| **范围外话题** | 礼貌挡回 + 引导（不答与经营无关的问题） |

**三层记忆（别混）**：

- **展示的对话（她能滚动看到的历史）**：进入页面默认加载最近一页，像微信一样向上滚动时按稳定游标继续加载更早消息；加载旧页必须保持当前阅读位置。消息带时间戳，按 seller/operator 隔离并服务端持久化，不依赖 localStorage，换设备/重装仍可读取。聊天只做**有界留存**：按单个 seller/operator 设置容量上限并批量淘汰最早消息，具体阈值在实现规格中结合试用数据确定；不再按固定“两天窗口”删除。到达已保留历史的起点时提示"更早的对话已清理"。聊天清理与业务订单、菜单、履约零级联。
...
- 当前首轮人工试用不提供餐次归档入口；是否需要“完成后只读 / 受控重开”由真实使用结果决定，不与到账记录绑定。
...
- **贴接龙（主）**：整段粘进来，AI 解析成候选订单（**日期 + 餐次 + 顾客 + 份数**）。格式是软的：整天、半天、只一餐都可能，由 LLM 帮忙拆分午/晚。标题里的日期/周几决定订单范围；菜单正文只是此前复制给顾客看的内容，解析时忽略，不与系统当前菜单比对。
- **私聊补单**：一句话也可以新增或改量，但必须信息完整，例如"7 月 3 日晚餐，加王阿姨 2 份"。`加 N 份`表示在当前数量上追加 N；`改成/总共 N 份`表示把最终总数设为 N。即使原本没有订单也按同一规则从 0 计算；确认卡分别展示`当前 1 + 2 → 共 3`或`当前 3 → 改成 2`。缺日期/餐次/顾客/份数/动作时，不写库，回补全卡。
...
- 同一卖家、顾客、日期、餐次是 active 订单的业务唯一坐标。再次粘贴同一天同餐的**完整接龙**时，以最后一次接龙为准：覆盖标题范围内的全部 active order，不区分这些订单此前来自接龙还是自然语言补单；已取消的历史单不阻止重新下单。
- 解析确认卡逐行标出"新增 / 更新 / 退出 / 不变"及改前、改后数量；桃子确认前不改任何订单。例如此前自然语言补了赵 1 份，而新完整接龙没有赵，确认卡应显示赵将退出，确认后才取消。
- 自然语言补单只修改明确指定的日期 + 餐次 + 顾客这一处坐标，不取消或改动同日其他订单。
- 如果现有订单已 confirmed，更新前必须二次确认，并明确会同步影响订单台账、送餐清单和到账记录汇总；若已经 paid、历史兼容值 reconciled 或 fulfillment=done，则完整接龙不得自动更新或取消，提示桃子单独处理。

**确认 / 写入**

- 解析结果先以确认卡展示；用户确认后，完整接龙或单笔补单的全部变更一次性写入 `draft` / 更新现有 order；旧确认卡遇到订单已变化时失效并要求重新预览，不留下部分写入。
- 「确认订单」把 draft → confirmed：进入订单台账、未标记到账汇总，并为该 order 建立 fulfillment，进入送餐清单。
...
- 老顾客的新订单自动带出当前默认地址字符串；新客地址选填，可以留空直接确认，填写后供后续订单自动带出。无地址订单仍可确认和履约，在送餐清单归入“无地址”兜底组。
- 重复粘贴同一天同餐时，以新完整接龙整体覆盖范围内所有录入来源，并在确认卡里标明新增/更新/退出/不变。
- 自然语言`加 N 份`和`改成/总共 N 份`分别按追加量和最终总数执行，确认卡展示当前值、运算和最终值，且不影响未提及订单。
...
- **① 打包/送餐清单（源头防错）**：出餐后给一张"这一趟要送什么"的待送清单，展示顾客、份数、地址字符串、手工到账记录状态。
...
- 顾客 = **名字**（接龙里的称呼，如 王燕萍 / 小柠檬 / 李叔，作识别）+ **默认送餐地址字符串** + 默认份数 + 默认餐次。MVP 只做基础名字归一，低置信称呼变体不自动合并；人工纠错/合并的实际操作方式留给首轮使用验证，不假定当前已经具备。
- **地址是选填且学来的**：接龙里没有地址时可直接确认；她私信拿到后录一次（可语音），**之后该顾客的新订单自动带出**。默认地址改变只影响后续新订单，不追溯改写历史订单快照。地址暂时就是一个字符串，不拆楼栋、单元、房号；一直没有地址也不阻断记单、确认或履约。
- **不存电话**：以接龙名字 + 默认地址为准，不强制采集电话（脱敏）。
- **不设自家特殊标识**：如果桃子家也录单，就只是一个普通顾客名和普通订单。

### 6.5 到账状态记录

- **[MVP]** 只记录桃子的手工判断：每单可在“未标记到账 / 已标记到账”之间切换，未标记订单可汇总查看。界面统一使用“到账记录”表述；`unpaid / paid` 仅是内部兼容字段值。系统不接支付、不验证资金结果，也不提供第三个“已核对”用户状态。历史 `reconciled` 数据只按“已标记到账”兼容读取，不作为当前入口。
...
| 集合 | 关键字段 | 说明 |
|---|---|---|
| `customers` | `displayName`(识别键，不唯一；MVP 只做基础名字归一，低置信不自动合并，不加 aliases 字段)、`address?`(默认送餐地址字符串)、`defaultServings?`、`defaultOccasion?`(同 occasion 枚举) | 接龙名字为识别；地址不结构化，就是一个 string；不存电话；不设 `self` / 自家特殊标识；人工纠错能力在首轮试用后单独决定 |
...
| `orders` | `customer`、**`date`(=用餐日/service date)**、**`occasion`(同 occasion 枚举)**、**`status`(draft/confirmed/canceled,index = 记单生命周期)**、`source`(审计,见下枚举,不控流)、`placedAt`(录入时间，≠用餐日)、`note?`、`address?`(本单地址字符串快照)、**`totalCents`(汇总列，hook 回写)**、`paymentStatus`(当前入口只写 unpaid/paid；reconciled 为历史兼容值,index)、`paymentMethod?`、`paidAt?`、`idempotencyKey?`、`createdBy?`(→operators) | 一个 order = 一个人的一天一餐；active 业务唯一坐标 = (seller, customer, date, occasion)，重复粘贴同坐标时更新现存 draft/confirmed order，canceled 历史单不占坑 |
...
> **到账记录内联进 `orders`**（paymentStatus/paymentMethod/paidAt），不建 payments 表，也不代表系统处理支付。当前用户入口只写 `unpaid`（未标记到账）/`paid`（已标记到账）并允许双向修改；`reconciled` 仅为历史兼容值，读取时等同 paid，不再提供写入口。到账记录轴始终独立于订单生命周期轴。
> **记单生命周期（与会话解耦，§5.5 三层记忆：业务数据永久）**：贴接龙/口述解析后，先回确认卡；用户确认后落 `status=draft` 的 order + items，或更新现有同坐标 order。「确认订单」把 draft→`confirmed`——**这步才物化经营状态**：upsert 开餐 slot + 建 fulfillment，并计入送餐/未标记到账等经营口径；「取消」→`canceled`（其 fulfillment 一并置 `status=canceled`、退出送餐/缺口口径）。**draft = 纯记录、零副作用**（不开 slot、不建履约、不进任何派生口径），所以错解析/没确认的草稿不会污染"今天该做"；「订单」tab 随时找回续确认。
...
- **派生不落表**：送餐地址排序、"最近一餐"聚焦、未标记到账汇总、"今天还差什么"——都是 kith-inn-be 确定性纯函数（Tech Spec §4 阶梯0、§6 单测 100%）。
...
- **§5.5 「今天」主对话 agent（MVP 核心）**：自然语言入口，编排下列工具 + 订单结构化；三层记忆 + 最近页/上翻分页与有界聊天留存；范围外挡回。
- §6.1 对话式记单（贴接龙 / 口述 → 智能分午晚 → 确认订单；地址从顾客带出）。
- §6.2 AI 菜单生成（菜品池、N 天不重复、整周批量、发布文案）。
- §6.3 防漏送 / 防错送（按地址字符串排序的送餐清单 + 地址片段/单条勾销 + 缺口对账）。
- §6.4 轻量顾客（自动沉淀、地址学来的）；§6.5 到账**状态记录**（纯手动，不接支付；催收提醒属 M2）。
- 三个详情 tab（菜单 / 订单 / 送餐）：确定性视图 + 常用快捷操作 + 可选日期；首轮试用不提供餐次归档入口。
- 顾客侧零改动，免费试用。
...
- **隐私**：顾客以"称呼 + 地址字符串"识别，不强采姓名电话。业务数据导出/删除属于独立数据权利 backlog，正式扩大外部使用前再确定范围、格式和删除语义；不默认作为首轮人工试用或部署前阻塞项。
