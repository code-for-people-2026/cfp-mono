...
> 本文是 [`PRD.md`](./PRD.md) §7 的解释版，按 v1.13 目标模型描述。代码层已按本文同步；数据库结构迁移清单见文末。
...
| 字段 | 业务意义 |
|---|---|
| `displayName` | 接龙里的称呼；不唯一，MVP 只做基础名字归一，低置信不自动合并；人工纠错入口由首轮试用决定 |
...
| `totalCents` | 派生总价 |
| `paymentStatus` | 当前入口只写 unpaid / paid；reconciled 是历史兼容值，读取时按 paid 展示 |
| `paymentMethod?` / `paidAt?` | 用户手工到账记录；不代表系统接入或验证支付 |
...
Payload 内置 `createdAt` 与主键 `id` 共同组成稳定分页游标：按 `(seller, operator)` 取最近一页，向上滚动时用 `(createdAt,id)` 加载更早页并避免同时间戳消息重/漏。**留存策略**（PRD §5.5）改为按 seller/operator 的容量上限有界保留，不再按固定两天裁剪；阈值在实现规格中结合试用数据确定，超限时批量淘汰最早页，且与业务表零级联。当前代码只有最近页读取，cursor 与 GC 尚未实现。`card` 是历史快照而非实时视图；新顾客确认卡的 reload-safe 动作状态（active/stale/completed）不在此字段，留给后续 feature。
...
confirmed order 仍可在桃子确认后更新；snapshot 退出 confirmed order 时，同时把其 pending fulfillment 置为 canceled。`paymentStatus=paid`、历史兼容值 `reconciled` 或 fulfillment 已 done 的订单属于既成事实，预览和确认阶段都拒绝批量变更，整批不留下部分写入。
