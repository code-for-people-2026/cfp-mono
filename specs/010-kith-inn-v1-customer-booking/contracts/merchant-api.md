...
请求只接受 `{ "text": string }`。首个非空行必须为 `YYYY-MM-DD 午餐|晚餐`，其余非空行必须为可选列表序号加“称呼 正整数份数”；最多 100 个数据行、原文最多 10,000 字符。任一数据行非法、餐次不存在或无法在当前 seller 下唯一匹配时返回 422，写入数为 0。
...
BE 必须重新解析原文、计算 SHA-256、重查当前 seller/meal slot 和价格；`confirmed` 非 true、hash 不匹配或餐次当前不可用均不得写入。每个数据行写入 draft `jielong-import` 订单，profile/openid/address 固定为空。`previewHash + 数据行序号` 作为 CMS 内部幂等标记；相同请求顺序重试返回 `existing`，不增加订单。
