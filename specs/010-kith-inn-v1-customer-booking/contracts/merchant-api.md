...
preview hash 是 seller、餐次身份、当前单价和 canonical input 的 SHA-256。BE commit 必须重新解析原文并重查同一 seller/meal slot。它先按请求中的 `previewHash + 数据行原始行号` 查询内部标记：若全部数据行都已存在，则在读取当前单价和重算 hash 前直接返回 `existing`，保证首次提交成功但响应丢失后，即使单价已变化，原请求仍可幂等重试；只要有数据行尚不存在，才重读当前单价并重算 hash，单价变化或 hash 不匹配时整批拒绝且不新增订单。匹配餐次只需是当前 seller 下已有且能取得有效服务端单价的 meal slot，不复用顾客登记的 batch、open/closed 或 deadline 门禁；无法取得有效单价返回 `409 meal-slot-unavailable`。`confirmed` 非 true 同样不得写入。每个数据行写入 draft `jielong-import` 订单，profile/openid/address 固定为空。
