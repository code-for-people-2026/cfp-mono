...
实现继续复用现有 v1 workspaces 和共享 `apps/cms`。M2-A/B、C1～C6、D1～D4、D2R 与 M3/M4 自动化切片均已按“nullable 导入读模型 → strict parser → CMS persistence → BE endpoints → FE fallback → 文案/隐私 → 数据控制 → 状态打磨 → 总验收”顺序完成。商家“今日工作台”目标采用“保留业务内核、逐页重写呈现层”，并已按纯状态模型、真实数据页面、专用手动加单三片串行完成；T028/T087/T090/T097 人工发布门禁和 T057 历史预算事实保持未完成。
...
**Scale/Scope**: 1 个实际 seller、微信群低并发顾客；自动化覆盖至少 2 seller × 2 openid；每批次/顾客提交最多 20 个坐标，单次接龙最多 100 个数据行；M2～M5 可自动化实现已完成，真机、微信后台与审核门禁仍独立保留
...
- 商家登录和已有 session 目前默认进入今日工作台，订单页保留筛选、汇总与状态动作，手动加单已拆为专用页面；实现保留原 API client、session store 与可复用纯逻辑，没有创建平行前端应用。
...
- **IV. 最小可交付与可审查切片**: 通过。M2 已完成；M3/M4 已按十个依赖有序切片交付；M5 已按规划、纯状态模型、真实数据页面、专用手动加单拆为四片，每片一个核心不变量并独立验证。
...
| PR | 单一目标 / 核心不变量 | 精确路径 | 独立验证 | 合并事实 / 人工 diff |
|---|---|---|---|---|
| M5-P | 只记录呈现层重构需求、决策与依赖有序任务，不修改业务代码 | `specs/010-kith-inn-v1-customer-booking/{spec.md,plan.md,tasks.md}` | 用户故事/FR/SC/Task 追踪、文档链接、`git diff --check`、`pnpm verify` | 已合并 PR #257；121 行 |
| M5-H1 | 建立可测试的今日工作台状态模型，不注册或渲染新页面 | `apps/kith-inn-v1-fe/src/logic/merchantHome.{ts,test.ts}` | FE 100% coverage；Asia/Shanghai 跨日、截止时间等于当前时刻、五种餐次状态、订单汇总、价格文案和手动加单资格 | 已合并 PR #258；250 行 |
| M5-H2 | 商家登录后进入使用真实数据的高保真今日工作台 | `apps/kith-inn-v1-fe/src/{app.config.ts,app.css,components/MerchantNav.tsx,logic/login.ts,logic/login.test.ts,logic/merchantHome.ts,logic/merchantHome.test.ts,pages/merchant/login/index.tsx,pages/merchant/home/index.tsx,pages/merchant/menu/index.tsx,pages/merchant/offerings/index.tsx,pages/merchant/orders/index.tsx}`、`apps/kith-inn-v1-fe/tests/e2e/{merchant,jielong-import}.spec.ts` | 登录/session 落点、午晚餐状态、待确认汇总、空态/部分失败/重试、快捷导航、FE coverage、无头 H5 E2E、weapp build | 已合并 PR #259；477 行，PR 已记录不可再拆原因 |
| M5-H3 | 用专用页面显式完成未开放、开放、截止和关闭餐次的 manual draft 补录 | `apps/kith-inn-v1-fe/src/{app.config.ts,app.css,logic/orders.ts,logic/orders.test.ts,pages/merchant/home/index.tsx,pages/merchant/orders/index.tsx,pages/merchant/orders/add/index.tsx}`、`apps/kith-inn-v1-fe/tests/e2e/merchant.spec.ts` | 已有/新建资料、正整数份数/备注、重复 manual draft 显式更新、manual canceled 显式重提、customer-card 冲突转查看既有订单、四种餐次状态、无餐次提示、FE coverage、无头 H5 E2E、weapp build | 已合并 PR #260；657 行，PR 已记录不可再拆原因 |

M5 各片均从前一片 rebase merge 后的最新 `main` 开始，没有 stacked。M5-H2 只编排 `listMealSlots(today, today)` 与存在餐次的 `listOrders`；M5-H3 只复用既有 profile/manual-order contract，未修改 BE、CMS、shared 或数据库结构。
