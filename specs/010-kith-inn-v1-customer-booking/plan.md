...
- 顾客新建、修改和取消继续由服务端截止门禁控制；只要餐次存在，商家在开放、已截止或已关闭状态都可手动补录私信订单。
- “标已付”仍只表示线下付款记录；不增加聊天、AI 主链路或在线支付。

| PR | 单一目标 / 核心不变量 | 精确路径 | 独立验证 | 依赖 / 人工 diff |
|---|---|---|---|---|
| M5-P | 只记录呈现层重构决策与依赖有序任务，不修改业务代码 | `specs/010-kith-inn-v1-customer-booking/{plan.md,tasks.md}` | 文档链接/任务状态、`git diff --check`、`pnpm verify` | latest `origin/main`；默认 `<400` |
| M5-H1 | 建立可测试的今日工作台状态模型，不注册或渲染新页面 | `apps/kith-inn-v1-fe/src/logic/merchantHome.{ts,test.ts}` | FE 100% coverage；Asia/Shanghai 跨日、五种餐次状态、订单汇总、价格文案和手动加单资格 | M5-P；目标 `<300` |
| M5-H2 | 商家登录后进入使用真实数据的高保真今日工作台 | `apps/kith-inn-v1-fe/src/{app.config.ts,app.css}`、`src/pages/merchant/home/index.tsx`、必要的既有登录/导航组件及对应测试、`tests/e2e/merchant.spec.ts` | 登录/session 落点、午晚餐状态、待确认汇总、空态/部分失败/重试、快捷导航、FE coverage、无头 H5 E2E、weapp build | M5-H1；默认 `<400`，略超须说明 |
| M5-H3 | 用专用页面显式完成开放、截止和关闭餐次的 manual draft 补录 | `apps/kith-inn-v1-fe/src/{app.config.ts,app.css}`、`src/pages/merchant/orders/add/index.tsx`、必要的 service/logic 与对应测试、`tests/e2e/merchant.spec.ts` | 已有/新建资料、正整数份数/备注、重复 draft 显式更新、canceled 显式重提、三种餐次状态、无餐次提示、FE coverage、无头 H5 E2E、weapp build | M5-H2；默认 `<400` |
