...
| M5-H3 | 用专用页面显式完成开放、截止和关闭餐次的 manual draft 补录 | `apps/kith-inn-v1-fe/src/{app.config.ts,app.css,logic/orders.ts,logic/orders.test.ts,pages/merchant/home/index.tsx,pages/merchant/orders/index.tsx,pages/merchant/orders/add/index.tsx}`、`apps/kith-inn-v1-fe/tests/e2e/merchant.spec.ts` | 已有/新建资料、正整数份数/备注、重复 draft 显式更新、canceled 显式重提、三种餐次状态、无餐次提示、FE coverage、无头 H5 E2E、weapp build | M5-H2；默认 `<400` |
