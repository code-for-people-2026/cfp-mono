...
实现继续复用现有 v1 workspaces 和共享 `apps/cms`。M2-A/B、C1～C6、D1～D4、D2R 与 M3/M4 自动化切片均已按“nullable 导入读模型 → strict parser → CMS persistence → BE endpoints → FE fallback → 文案/隐私 → 数据控制 → 状态打磨 → 总验收”顺序完成；T028/T087/T090/T097 人工发布门禁和 T057 历史预算事实保持未完成。
...
**Scale/Scope**: 1 个实际 seller、微信群低并发顾客；自动化覆盖至少 2 seller × 2 openid；每批次/顾客提交最多 20 个坐标，单次接龙最多 100 个数据行；M2～M4 可自动化实现已完成
