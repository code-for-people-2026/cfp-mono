...
在不改变后端、共享契约和现有路由的前提下，把商家首页收敛为“品牌与问候 → 待确认提醒 → 午晚餐状态 → 固定底部导航”，不再复制菜单、订单或配送入口。可单测的纯逻辑继续生成问候、菜单摘要、价格、截止时间和状态文案；页面组件继续负责 session、请求编排、局部失败隔离、竞态保护和上下文跳转；全局样式中的首页与 `MerchantNav` 区段负责奶油色视觉、统一卡片、触控目标和安全区。
...
**Constraints**: 不改后端/API/共享契约；不增加 mock 或静态业务数据；不实现头像、伪系统胶囊、“我的”、聊天、AI、支付；不把待配送状态包装为独立模块；固定导航不遮挡小屏最后一张餐次卡片及其操作

**Scale/Scope**: 一个商家首页、午餐/晚餐两个固定视图、五种餐次状态、餐次上下文动作和一个共享底部导航
