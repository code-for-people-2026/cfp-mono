...
- `CI=1 pnpm --filter @cfp/kith-inn-fe test:e2e:mainline` 连续三次均为 4/4，通过耗时 21s、21s、22s；复审修正后再跑 4/4，通过耗时 21.8s。
- package script 会在 Playwright 进程启动前一次性清空 service 目录；停止 PostgreSQL 的受控 webServer failure 验证旧 sentinel 全部消失且只留下本次 `cms.log`，其中明确记录 `ECONNREFUSED 127.0.0.1:54324`。
