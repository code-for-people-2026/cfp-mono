...
## 决策 3：预览使用单调版本，提交期间锁定快照

- **Decision**：非提交期间原文每次变化都推进版本；预览捕获原文快照与版本，只允许当前版本更新 UI。提交前验证预览版本匹配，发出 commit 后锁定原文和冲突选择直至请求结束。
- **Rationale**：冲突选择按行号解释，陈旧预览可能把覆盖选择应用到另一道菜；commit API 收到文本后会实际写入，客户端仅忽略过期响应无法撤销旧文本造成的服务端变更。
- **Alternatives considered**：仅在输入事件中清空预览不能阻止飞行中的旧响应重新写回；允许提交中继续编辑再隐藏旧结果仍会写入旧快照；后端 preview token 能提供服务端版本保护，但会改变本次明确不修改的 API 契约。
