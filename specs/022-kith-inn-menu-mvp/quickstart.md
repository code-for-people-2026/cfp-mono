...
会话、健康、菜池及周菜单接口均已实现。由维护者在服务端安全配置表内真实值；本指引不提供会把密钥打印到终端的读取命令。启动 API 和前端开发服务分别运行：
...
本地 H5 联调通过同源代理转发 `/api/kith-inn/`；前后端分别运行并不自动解决浏览器跨域限制。API 当前没有开放 CORS。后续 H5 部署优先保持同源，若采用不同源，必须在 #360 中验证精确允许的 Origin 与预检请求；不能使用允许任意来源的配置代替。反向代理场景的登录限流地址识别也在部署前核验，不能直接信任任意 `X-Forwarded-For`。
