...
| #376 最新复核：仓库没有声称的 H5 开发代理 | 已纠正文档：单独启动命令不构成真实 H5 联调环境。当前用户试用依赖仓库外临时验收启动器；仓库可复现的是受控 HTTP 的 H5 回归和独立 PG API 测试。可共享 HTTPS/同源代理/测试会话配置明确留在 #360，不再把本机已发生的联调写成拉取仓库即可启动。 |
