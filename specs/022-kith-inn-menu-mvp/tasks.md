...
| PR9 | 复制文字与已保存的一餐一致 | US-12；AC-24；联合路径 | T018、T019 | apps/kith-inn-miniapp/src/ 菜单文字；tests/e2e/；playwright.config.ts；不代发微信，不提供公开订单入口 | 固定日期/午晚餐/菜名快照、复制失败及重开链路 | 350～550 | PR8 |
...
- [ ] T018 [PR9] 在 `apps/kith-inn-miniapp/src/lib/menu-text.ts`、`src/lib/menu-text.test.ts`、`src/pages/week/index.tsx` 实现重新读取保存周后的单餐文字预览、主动复制和失败重试；读取失败不使用旧缓存宣称最新；文字含日期、午晚餐、菜名，遵守去汤快照；复制不改菜单、不等于已发送，复制或发群后允许编辑保存并重新复制；旧微信文字不会随菜单修改更新，由桃子自行通知；按用户最新反馈，不再添加相应页面解释提示；不加公布或解锁动作。
...
2026-09-21 用户确认以排好菜作为主流程和导航依据。#358 在 `b385ec1` 完整基线上复用 T015/T017/T019，新增增量 PR，允许修改 `src/lib/main-nav.tsx`、小程序页面、共享样式/按钮、原型资产及必要回归和视觉 QA；协调者更新 PRD/spec/plan/tasks/handoff。验收以 [spec.md 第 1.1 节](spec.md#11-本轮界面验收)为准，特别覆盖草稿离开取消、未知写入状态、分类预览保留、历史刷新及独立复制流程。目标人工增量 500～750 行（含文档），超过 800 行按既有映射拆片；不再分配 Task ID，不改变接口和数据库。

后续版本纠正：用户明确工程原型最新，之前混用客户版是实施方错误。同一 #358 / T017/T019 从 `7ca12b5` 修正为工程版首页/编辑/候选/确认/历史层级、两列表格及同尺寸视觉；允许在既有小程序 `src/lib/` 提取复用的表格、候选等组件，必要交互回归仍在原测试范围。必须验证检查页不写入、最终一次保存确认、候选返回和取消不改目标、跨目标不误换、历史周边界及旧草稿/pending保障。规格第 1.1 节优先于旧三列描述，单片预算和任务映射沿用计划。
