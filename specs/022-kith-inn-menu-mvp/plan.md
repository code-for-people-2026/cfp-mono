...
PR1 在独立审查后补充 UUID 身份去重和按错误码要求必要详情，并同步 OpenAPI 与反例，预算调整至 350～500 行，仍保持单一契约目标。PR2 的数据库约束、迁移和真实事务反例需要一起证明持久化边界；PR3 的令牌生命周期、真实 socket 拒绝路径及真实 PG 就绪反例共同构成权限边界，预算调整为 400～650 行；PR4 的原子批量写与幂等重放必须在同一事务验证。PR6 将快照、版本和幂等归于一个周保存事务，PR7/PR8 把页面及其独有状态转换配套验证；PR10 包含运行配置、恢复流程与实际交接证据。因此这些片允许超过 400 行参考值，目标控制在各行预算内。没有用“跨层闭环”合并前后端，独立算法已放在 PR5；公共配置或边界继续增长时应再拆。
