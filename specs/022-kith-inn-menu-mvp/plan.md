...
日期：2026-09-22 · 状态：菜池、周菜单、复制核心闭环及用户反馈已可本地试用；集成 PR #376，真机及上线仍待验收 · 来源：[功能规格](spec.md)、[数据模型](data-model.md)、[接口契约](contracts/openapi.json)

合并安排：用户已授权将本地 H5 MVP 合入主线。PR #376 已完整包含 #361～#367、#369～#375 的有效代码；仓库只允许 squash，改为一次集成合并并将旧切片标为被覆盖，不改变既有任务和业务范围。独立运维 PR #368 保留。最终提交、评审处理和验证以 [合并核查记录](checklists/merge-review.md) 及 PR 状态为准；下文的分片记录用于追溯，不作为逐个合并要求。
...
- Q1～Q3 已按用户 1A、2A、3A 确认：改结构提示覆盖后重生成，取消/失败保留旧结果；不足一餐整次不生成；确认或分享后均可编辑保存并重新复制，由桃子自行通知邻居，不展示旧消息相关解释提示，不建设公布锁。具体规则见 [spec.md](spec.md)。
...
2026-09-21 用户要求工程交付减少反复查漏。本节约束后续变更，不增加产品范围或新的计划/任务编号。

- 开工前，实施者从本规格及固定版本工程原型整理本次受影响的状态、操作、结果和验证方法，复用现有 Issue / design-qa 记录。原型中的动作和反馈属于实现合同，不能只提取静态布局；不要求用户重复列出原型已有行为。
- 允许的 MVP 差异按 spec 1.1 / PRD 附录 A 执行。未授权增删产品行为或文案不得自行实施；已明确的用户指令优先，并同步冲突的旧文档。普通实现方式自主决定，只有新增业务取舍需要用户决定。
- 每项交付要求必须关联验证证据：交互以实际操作和结果为准，截图以同尺寸、同状态为准；测试从要求出发并能暴露已知错误，不能只断言实现存在或CSS属性。此次横向表至少覆盖鼠标/触摸滚动结束、列起点、末尾完整两天、手势取消和拖动不误选菜；文案删除检查编辑及复制相关路径。
- 沿用根 pnpm verify、适用浏览器回归、最新提交CI及现有审查。发现的可复现缺陷必须收敛并补有价值的回归，不因交付措辞、静态截图或已有测试通过而跳过。
- 在现有 design-qa 中记录“要求 → 操作/测试 → 结果/证据 → 未验证范围”。按平台明确H5实测、WeApp构建与微信真机实测的区别。未验证项不得宣称完成；局部开发通过不得概括为整个原型、MVP或真实微信已验收。
- 用户负责业务选择与最终产品接受；基础交互、数据保护和原型一致性由实施者在交付前检查。已知缺陷、真实环境待办及用户未接受状态继续保留，不以代码完成替代。

2026-09-24 US-01 / AC-01 补充：单菜编辑提供确认删除，端到端接通 DELETE /dishes/{dishId}（baseVersion、幂等重试），验证历史快照不变、同名重加及取消／失败保护；沿用菜池维护切片，不恢复旧订餐范围。
