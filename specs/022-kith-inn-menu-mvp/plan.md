...
日期：2026-09-20 · 状态：开发准备稿，尚未实现 · 来源：[功能规格](spec.md)、[数据模型](data-model.md)、[接口契约](contracts/openapi.json)

本计划只交付菜池、排周菜单、保存回看和单餐文字复制。经营者只有桃子；订单、对账、连续订餐继续在首版之外。当前任务填写规格，不创建应用、服务、GitHub 事项或发布。

## 1. 工程现状与选择

工程事实固定核对至 `origin/main@7bb984d6697b74b1f095cb6475be6cb0f4d0e264`，不是把旧本地分支当作线上状态。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PR1 | 建立唯一可执行契约 | 全部首版故事；AC-01～05、24、25 | packages/kith-inn-contracts/；knip.json；pnpm-lock.yaml | 不实现服务或页面 | 合法/非法 DTO 与 OpenAPI 示例一致 | 250～400 | 无 |
...
| PR3 | 只有桃子可取得和使用会话 | 全部首版故事；AC-25 | apps/kith-inn-api/src/ 会话、HTTP、运行入口及测试 | 不自动注册经营者或接入其他产品账户 | 非白名单拒绝、过期/撤销拒绝、日志无凭据 | 400～550 | PR2 |
| PR4 | 菜池写入原子且安全重试 | US-01；AC-01 | apps/kith-inn-api/src/ 菜池、幂等及路由测试 | 不在服务端分类，不实现菜单生成 | 重名整批回滚、版本冲突、同键重放与异体拒绝 | 300～450 | PR3 |
...
| PR7 | 桃子可在小程序维护菜池 | US-01；AC-01、25 | apps/kith-inn-miniapp/ 工程、客户端、菜池页面；knip.json；turbo.json；.gitignore；pnpm-lock.yaml | 不建设顾客端或订单 | 分类循环纠正、未确认不写入、鉴权失败可恢复 | 450～600 | PR4 |
| PR8 | 一周排菜调整与保存回看可用 | US-02、03a、03b、03c；AC-02～05 | apps/kith-inn-miniapp/src/ 周编辑状态、菜单及历史页面 | 不将内存状态或缓存当作已保存 | 指定位置换菜、逐餐去汤、保存失败保留编辑、冲突提示 | 450～600 | PR6、PR7 |
| PR9 | 复制文字与已保存的一餐一致 | US-12；AC-24；联合路径 | apps/kith-inn-miniapp/src/ 菜单文字；tests/e2e/；playwright.config.ts | 不代发微信，不提供公开订单入口 | 固定日期/午晚餐/菜名快照、复制失败及重开链路 | 250～400 | PR8 |
...
PR2 的数据库约束、迁移和真实事务反例需要一起证明持久化边界；PR3 的令牌生命周期与拒绝路径共同构成权限边界；PR4 的原子批量写与幂等重放必须在同一事务验证。PR6 将快照、版本和幂等归于一个周保存事务，PR7/PR8 把页面及其独有状态转换配套验证；PR10 包含运行配置、恢复流程与实际交接证据。因此这些片允许超过 400 行参考值，目标控制在 600 行内。没有用“跨层闭环”合并前后端，独立算法已放在 PR5；公共配置或边界继续增长时应再拆。

可并行部分：PR2 完成 API 包与测试脚手架后，可并行完成 PR3 和 PR5；PR4 后可并行准备 PR6 和 PR7。其余按依赖列执行，不在依赖未完成时把 Mock 联调视作生产验收。
