...
**Input**：`specs/018-kith-inn-v1-merchant-offerings-hifi/` 下的 spec、plan、research、data-model、contracts 和 quickstart
...
|----|-------------------|---------------|----------|-----------------|----------|-----------|------|
| PR1 | 固化 Page 2 行为、竞态约束和执行计划 | US1-US4、FR-001~015 | T001-T003 | `specs/018-*/**`；不改运行时代码 | checklist 全通过、Task ID 映射检查 | 约 500 行 | 无 |
| PR2 | 保证异步响应与编辑合并不破坏当前用户意图 | US1-US3、FR-003/004/006/008~010/015 | T004-T012 | `src/logic/offeringsView*`、`src/pages/merchant/offerings/index.tsx`；不做视觉换肤 | Vitest coverage、lint、typecheck、定向 E2E | 约 380 行 | PR1 |
| PR3 | 收敛 Page 2 页面结构且不回退真实管理流程 | US1/US2/US4、FR-001/002/005/012~015 | T013、T015 | `src/pages/merchant/offerings/index.tsx`、`tests/e2e/**`；不做最终视觉换肤，不改 API/CMS | 定向 E2E、lint、typecheck、双端 build | 约 500 行 | PR2 |
| PR4 | 完成 Page 2 高保真样式和窄屏验收 | US4、FR-001/002/012~015 | T014、T016-T017 | `src/app.css`、`quickstart.md`；不再改业务流程，不提交 Prompt | 354×786 视觉验收、`pnpm verify`、定向 E2E | 约 460 行 | PR3 |
...
- [x] T001 在 `specs/018-kith-inn-v1-merchant-offerings-hifi/spec.md` 与 `checklists/requirements.md` 定义并校验用户场景、边界和成功标准
- [x] T002 在 `specs/018-kith-inn-v1-merchant-offerings-hifi/plan.md`、`research.md`、`data-model.md`、`contracts/merchant-offerings-ui.md` 和 `quickstart.md` 记录 brownfield 事实、设计决策与验证方法
- [x] T003 在 `specs/018-kith-inn-v1-merchant-offerings-hifi/tasks.md` 建立完整 Task ID、story 与 PR slice 映射
...
- [ ] T013 [US4] 在 `apps/kith-inn-v1-fe/src/pages/merchant/offerings/index.tsx` 保留并收敛 Kimi 的浏览/管理/弹层 JSX、加载/空/错误状态和真实 API 入口
- [ ] T014 [US4] 在 `apps/kith-inn-v1-fe/src/app.css` 保留高保真主体并修复长菜名/主料挤压、disabled 预览按钮不可读及 Page 1 导航兼容
- [ ] T015 [US4] 在 `apps/kith-inn-v1-fe/tests/e2e/merchant.spec.ts` 与 `jielong-import.spec.ts` 覆盖新入口、管理分组、编辑顺序和导入失效回归
- [ ] T016 [US4] 在 `specs/018-kith-inn-v1-merchant-offerings-hifi/quickstart.md` 记录 H5/微信小程序 build 与定向 E2E 的实际结果
- [ ] T017 [US4] 在 `specs/018-kith-inn-v1-merchant-offerings-hifi/quickstart.md` 记录 354×786 默认态、管理态、批量导入态的人工视觉验收及新增/编辑弹层的 E2E 验收事实
...
- T013/T014 依赖 T006、T009、T012，避免视觉改动掩盖正确性问题。
- T015 依赖 T013/T014；T016/T017 依赖全部运行时代码和测试任务。
- PR 实际合并顺序固定为 PR1 → PR2 → PR3 → PR4；即使不同 story 的测试可并行编写，也不得绕过该顺序发布。

## 并行机会

- T004、T007、T010 都修改同一测试文件，实际按顺序执行以避免冲突。
- PR2 合并后，T013 的 JSX 收敛与 T014 的 CSS 修复位于不同文件，可并行检查，但最终视觉验收必须基于二者共同结果。
...
3. 保留 Kimi 已通过验证的页面骨架与样式，修复 review 发现的窄屏可读性问题。
