...
把 kith-inn 自动换菜从“硬过滤避重、筛空即失败”改为“保留全部有效同类候选，按四级冲突计数字典序择优，必要时显式返回放宽规则”。领域内核先形成可注入随机源的纯函数；路由随后读取目标日前 7 日与目标自然周的 seller-scoped menu plans、排除当前 plan，并把解释传给 H5；最后用最小 Playwright 场景锁定小池仍可换和提示可见。
...
**Scale/Scope**: 单 seller 的日常菜品池与最多 14 个自然日餐次；一个 swap 端点、一个菜单页提示、一个 H5 E2E 场景
...
- **IV 单一切片**: 通过；规划、纯领域、服务/UI、E2E 四个 PR 各有一个可独立审查的不变量。
...
| PR2 | 纯领域内核对所有有效候选做固定四级择优，且结果可解释、随机可控 | `packages/kith-inn-shared/src/{schemas,types}*`、`apps/kith-inn-be/src/domain/menu/core*` | shared schema 测试、BE core 四类边界/优先级/稳定性测试、`pnpm verify` | PR1 |
| PR3 | swap 路由使用目标日期历史并把放宽解释无损显示到菜单页 | `apps/kith-inn-be/src/routes/menu*`、`apps/kith-inn-fe/src/{logic,pages}/**`、必要 shared 响应契约、`docs/kith-inn/**` | route 历史范围/当前 plan 排除/单位置 patch，FE 格式化测试，指定/published 回归，`pnpm verify` | PR2 |
| PR4 | 非 v1 H5 自动化真实覆盖“小池仍换 + 放宽提示” | `apps/kith-inn-fe/{playwright.config.ts,tests/e2e/**,package.json}`、必要 `.github/workflows/ci.yml`/lockfile | `CI=1 pnpm --filter @cfp/kith-inn-fe test:e2e`、`pnpm verify` | PR3 |

PR1 的全套规格文档预计略超 400 行，因为宪法要求 spec/plan/research/data-model/contracts/quickstart/tasks 同时可审查；它不包含源码，预计低于 800 行。PR2–PR4 各自控制在 400 行内；若 E2E 接线实际超预算，优先复用既有 webServer/CI 模式，不并入 #157 的 PostgreSQL/跨租户场景。
...
**Structure Decision**: 评分与日期边界留在既有纯领域内核；路由只负责 seller-scoped CMS 读取、plan 身份排除和写回；FE 纯 helper 负责规则→中文，页面只持有本次瞬时提示；E2E 复用仓库现有 CMS/BE/H5 进程编排。
