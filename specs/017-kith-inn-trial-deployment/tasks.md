...
- [x] T037 [US2] 在 `.github/workflows/deploy-production.yml` 与 `deploy/RUNBOOK.md` 实现 migration 前目标 RDS 可恢复备份创建/验证与非敏感 ID/时间记录、单次 migration/provision、seller ID 机器输出直传、Compose rollout、部署后 smoke；任一步失败自动恢复上一应用 digest，schema 不兼容时停止并转人工数据恢复
