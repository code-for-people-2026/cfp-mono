...
| PR4a | 生产 `cms` schema 只走提交的 migration，本地开发才允许 push | T017–T018 | fresh/existing PG、重复 migrate、零 reset、`pnpm verify` | PR3 |
| PR4b | 桃子基线幂等收敛且真实 OpenID 不进证据 | T019–T022 | seed 重跑/中断恢复、v1 sentinel、`pnpm verify` | PR4a |
| PR5 | CMS/BE/H5 形成同 SHA 可追踪、非 root、可启动镜像 | T023–T027 | 三镜像 build/health/secret scan、`pnpm verify` | PR4b |
...
- [x] T017 [US1] 已在 `apps/cms/tests/migration-production.test.ts` 固定 fresh DB、已迁移 DB、重复 migrate、生产禁止 push/reset 与 migration head 不匹配场景，并在隔离 PostgreSQL 实库通过
- [x] T018 [US1] 已在 `apps/cms/migrations/**` 生成并审阅 baseline，生产仅执行显式 migration、本地开发保留 push；CMS 实库全量测试与带合法构建 URL 的 `pnpm verify` 均通过
