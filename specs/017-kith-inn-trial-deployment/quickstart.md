...
docker build -f apps/cms/Dockerfile \
  ...
  -t kith-inn-cms:verify .
docker build -f apps/kith-inn-be/Dockerfile \
  --build-arg RELEASE_SHA="$RELEASE_SHA" \
  -t kith-inn-be:verify .
...
bash deploy/verify-kith-inn-images.sh \
  "$RELEASE_SHA" kith-inn-cms:verify kith-inn-be:verify kith-inn-h5:verify
```
