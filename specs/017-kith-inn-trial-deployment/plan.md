...
| PR4a | `cms` schema 只由提交的 migration 推进，本地开发才允许 push | `apps/cms/payload.config.ts`、`apps/cms/migrations/**`、migration head/readiness | fresh/existing PG migration、重复执行、零 reset | PR3 |
| PR4b | 桃子基线可幂等收敛且真实 OpenID 不入库外证据 | `apps/cms/seed/**`、`packages/kith-inn-payload/src/seed/**` | seed 两次/中断恢复、v1 sentinel | PR4a |
| PR5 | CMS、BE、H5 均生成同一提交可追踪、非 root、可启动的生产镜像 | 三个 app 的 `Dockerfile`、`next.config.ts`、`.dockerignore` | 逐镜像 build、非 root/health、secret 扫描 | PR4b |
...
PR1 的全套 Spec Kit 产物预计处于 400–800 行：`spec/plan/tasks` 与 research/contracts/quickstart 必须在同一 PR 原子 review，拆开会使 analyze 缺少输入或留下互相失配的占位；本 PR 仍控制在 800 行以内。PR4a 的 Payload baseline migration 属明确机器生成文件，不计人工 diff；实测 migration 与 seed 合并后人工 diff 为 475 行，因此按预设条件拆出 PR4b。其余每片默认人工 diff <400 行，超过时必须再按表内不变量拆分。
