...
- 入口用 shared `offeringUpdateSchema` 校验：只接受 `name?`/`mainIngredient?`/`category?`；空对象 → 400；`name` 给了须非空、`category` 给了须合法。
- find-then-update 确认归属：先 `payload.find({ where: { and: [{id}, {seller: sellerId}] } })`，无命中 → 404（跨租户不暴露存在性）。
...
- find-then-update：先按 `(id, sellerId)` find，无命中 → 404。
