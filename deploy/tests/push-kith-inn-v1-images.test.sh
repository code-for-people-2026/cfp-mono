...
script="$root/deploy/push-kith-inn-v1-images.sh"
...
run_push() {
  ...
  env \
    RELEASE_SHA="$sha" \
    KITH_INN_V1_CMS_IMAGE="registry.example/cfp/kith-inn-v1-cms:$sha" \
    KITH_INN_V1_CMS_OPS_IMAGE="registry.example/cfp/kith-inn-v1-cms-ops:$sha" \
    KITH_INN_V1_BE_IMAGE="registry.example/cfp/kith-inn-v1-be:$sha" \
...
if env \
  ...
  KITH_INN_V1_CMS_IMAGE="registry.example/same:$sha" \
  KITH_INN_V1_CMS_OPS_IMAGE="registry.example/same:$sha" \
  KITH_INN_V1_BE_IMAGE="registry.example/be:$sha" \
...
echo "kith-inn-v1 image push tests passed"
