...
run_selector() {
  ...
  : > "$output"
  GITHUB_SHA= DEPLOY_BASE="$base" DEPLOY_HEAD="$head" \
...
base="$head"
head="$(synthetic_commit apps/weekly-menu-be/.production-target-test 'test: weekly menu range')"
run_selector "$worktree" "$base" "$head" "$tmp/weekly-menu-range"
assert_output "$tmp/weekly-menu-range" false true
...
assert_output "$tmp/unrelated-range" false false
...
base="$head"
head="$(synthetic_commit deploy/weekly-menu-compose.fixture 'test: weekly menu deploy range')"
run_selector "$worktree" "$base" "$head" "$tmp/weekly-menu-deploy-range"
assert_output "$tmp/weekly-menu-deploy-range" false true
...
current_job="$(sed -n '/^  current:/,/^  prepare:/p' "$workflow")"
...
grep -q "should_deploy=true" <<<"$current_job"
grep -q 'needs: current' <<<"$prepare_job"
grep -q "needs.current.outputs.should_deploy == 'true'" <<<"$prepare_job"
grep -q 'needs: \[current, prepare\]' <<<"$deploy_job"
...
grep -q 'weekly_menu_affected:.*outputs.weekly_menu' "$ci_workflow"
