...
grep -q 'needs: \[affected, serialize_shared_rds, prepare, prepare_kith_inn\]' <<<"$deploy_job"
grep -q "needs.serialize_shared_rds.result == 'success'" <<<"$deploy_job"
...
grep -q '^  serialize_shared_rds:$' "$workflow"
grep -q '^  serialize_shared_rds:$' "$v1_workflow"
grep -q 'wait-for-older-production-run.sh deploy-kith-inn-v1-production.yml' "$workflow"
grep -q 'wait-for-older-production-run.sh deploy-production.yml' "$v1_workflow"
grep -q 'needs: \[affected, serialize_shared_rds\]' "$v1_workflow"
