...
smoke() {
  KITH_INN_ENV_FILE="$1" RELEASE_SHA="$2" KITH_INN_PROVISIONED_SELLER_ID="$3" \
    ...
    KITH_INN_BE_BASE_URL="$(value "$1" KITH_INN_BE_BASE_URL)" "$smoke_bin" kith-inn
}
recover() {
...
smoke "$next_env" "$release_sha" "$seller_id" >/dev/null 2>&1 || recover smoke
...
jq -cn --arg releaseSha "$release_sha" --arg migrationHead "$migration_head" --arg sellerId "$seller_id" \
  '{releaseSha:$releaseSha,migrationHead:$migrationHead,sellerId:$sellerId,status:"passed"}'
