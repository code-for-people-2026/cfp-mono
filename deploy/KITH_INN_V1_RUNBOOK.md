...
- `Deploy kith-inn-v1 Production` 是独立 workflow；website workflow 不调用 v1，website-only 变更不会部署 v1，v1/CMS-only 变更也不会部署 website。两者使用各自的 concurrency group，不会互相淘汰 pending run。因为它们的 Postgres schema 虽隔离、RDS 恢复点却属于整个实例，确实要部署时，较新的 workflow run 会按 GitHub run ID 等待较旧的另一套生产 run 完成，再进入 gate/backup/migration/rollout，避免两个项目的恢复点和写入窗口重叠。旧版 `kith-inn` production target 已关闭，旧 BE/FE 变更不会再启动第二个 CMS。v1 构建共享 CMS runtime、CMS migration/provision job 和 v1 BE。截至本次审计，历史 Actions 没有 v1 BE smoke artifact；因此只能确认“当前 CI 未部署 v1”，不能据此排除人工部署。
- v1 领域模型支持多个 seller；当前产品入口只自动 provision 一个“桃子”seller 和一个 operator，尚无自助开店或运营后台。

## ECS 自动部署

相关代码合入 main 后，`.github/workflows/deploy-kith-inn-v1-production.yml` 独立监听 push，并由 `deploy/resolve-kith-inn-v1-production-target.sh` 只判断共享 kith CMS、v1 BE、它们的 workspace 依赖和 v1 部署契约；也可通过 `workflow_dispatch` 显式重发。`.github/workflows/deploy-production.yml` 只负责 website 和已关闭的旧版 target，不再调用 v1；website workflow 自身也可手动重发。两个 workflow 使用不同的 concurrency group，一个项目的 pending run 不会淘汰另一个项目；一个项目的变更也不会触发另一个项目的部署。只有命中各自部署范围后，`deploy/wait-for-older-production-run.sh` 才会为共享 RDS 排队；API 或排序检查失败时发布失败关闭。旧版 `kith_inn` 输出固定为 `false`，不要求其微信或部署配置。v1 workflow 还会等待同 SHA 的 `ci.yml`。缺任一 v1 配置时发布失败关闭，且不会产生 smoke marker；这不算一次已验证发布，必须补齐配置后重新触发。
