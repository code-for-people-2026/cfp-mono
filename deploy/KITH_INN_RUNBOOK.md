...
第一轮由协调者统一提供固定本地H5入口，使用受控测试身份及真实API/PG，集中完成菜池→周结构→生成→指定换菜/去汤→保存重开→复制文字；不等待AppID，不要求用户审代码。本任务不替换协调者的3317/3319环境。
第二轮再在现有微信小程序验证真实身份、合法域名、双设备及微信剪贴板。两轮按release-evidence分别记录，本地成功不计微信验收，也不代表公网运行或上线。
