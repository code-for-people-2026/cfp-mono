...
- secrets：`ALIYUN_ACR_*`、`ALIYUN_ACCESS_KEY_ID`、`ALIYUN_ACCESS_KEY_SECRET`、`ECS_*`、`DATABASE_URL`、`KITH_INN_PAYLOAD_SECRET`、`KITH_INN_JWT_SECRET`、`KITH_INN_CMS_INTERNAL_TOKEN`、`KITH_INN_TRIAL_OPENID`、`KITH_INN_WX_APPID`、`KITH_INN_WX_SECRET`、`KITH_INN_DEEPSEEK_API_KEY`；
...
job 等目标 SHA 的 `ci.yml` 通过后，先等同一 run 的 website deploy 成功或明确跳过，避免两个 job 并发操作同一 ECS Docker daemon；随后构建并推送四个同 SHA 镜像，记录 registry digest。候选文件和校验和固定的 `jq 1.7.1` 会先放到远端，但不改 active runtime；手动执行 rollout 时也必须预装 `jq`。紧接着进入发布维护窗口：先停止 CMS/BE/H5 阻断新写入，再创建恢复点。数据库 URL host 必须属于目标 RDS endpoint，且备份 task `Finished`、备份集 `Success` 与 `IsAvail=1` 后，才允许各执行一次 migration、provision 和候选 runtime；非敏感 backup ID/时间同时写入 GitHub job summary，失败恢复时无需临时搜索 RDS。备份失败或后续部署失败会尝试恢复 last-good runtime；write gate 自身失败时也会先恢复原 runtime 再退出。provision 的非敏感 seller ID 在同一远端进程直接传给 smoke，不是 workflow 输入。

权限为 `0600` 的 `.env.kith-inn` 始终表示 last-good runtime；候选只使用独立 `.env.kith-inn.next`，直到 readiness 与公开入口 smoke 全部成功后才原子提升为 current，并把原 current 保存为 previous。因此 Actions timeout、SSH 断开、host reboot 或进程被杀都不会把未验证候选写成回滚基线。候选 pull 前先尽力清理同一组 kith 仓库中的旧镜像，避免磁盘已满时连候选拉取都无法开始；current、previous 以及任意现存容器引用的 image ID 都受保护，无关仓库也不处理。candidate pull、migration、provision、readiness 或 smoke 失败时自动只恢复 last-good CMS/BE/H5，绝不运行旧 ops image 或猜测数据库回退。若首次部署没有 last-good，或 migration 开始后旧 runtime 无法通过新 schema 的 health，则停止候选并输出 `manual_recovery_required`，使用 job summary 中的恢复点转入下面的数据恢复分支。smoke 成功后再次执行同样的受保护清理，避免 40G ECS 被历史 digest/layer 持续占满。清理冲突会输出带阶段的 `stale_image_cleanup_failed` 告警，但不阻断候选流程，也不把已经通过 smoke 的 release 改判为失败。
