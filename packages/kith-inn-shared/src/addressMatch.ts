/**
 * 地址前缀匹配的 canonical（FE ordersLifecycle 内联副本按此 keep-in-sync；be 端的
 * `fulfillmentsMatchingAddress` 已随 agent mark_delivered 一并移除）。
 ...
 * 带字母（`3a`/`26B`）继续 startsWith（字母已消歧），**大小写不敏感**（`3a` 匹 `3A`）。
 */
