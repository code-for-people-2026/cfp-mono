...
describe("domain schemas — happy parse", () => {
  it("seller / operator / customer", () => {
    ...
    expect(customerSchema.parse({ id, displayName: "王燕萍", kind: "regular", seller: id })).toMatchObject({ kind: "regular" });
  });
...
describe("contract schemas", () => {
  ...
  it("cards", () => {
    ...
    const order = { id, customer: 7, date: "2026-07-02", status: "draft" as const, source: "chat-paste", paymentStatus: "unpaid", seller: id };
...
describe("rejections", () => {
  it("rejects unknown enum values", () => {
    ...
    expect(() => customerSchema.parse({ id, displayName: "X", kind: "alien", seller: id })).toThrow();
  ...
  it("rejects missing required fields", () => {
    ...
    expect(() => fulfillmentSchema.parse({ id, serviceDate: "x", mode: "delivery", seller: id })).toThrow(); // missing status + orderItem
  });
});
