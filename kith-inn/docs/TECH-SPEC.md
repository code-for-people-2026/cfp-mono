...
- **legacy 不复用 + `recipes` 后门**：`packages/menu-core`、`apps/community-cooking`、website 的 `recipes` 集合都是 **legacy，kith-inn 从 0 新建、完全不复用**（菜单引擎含主料层去重也是新写，不改 menu-core）。M0 把 website Payload 搬进 cms 时注意：`recipes`(`read:()=>true`、无 seller) 是隔离边界内的后门——随 legacy 清理移除，或迁移期间隔离掉；`offerings` 是带 seller 的独立新集合。
