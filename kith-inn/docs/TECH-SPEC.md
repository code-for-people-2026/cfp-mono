...
- **三层记忆（别混）**：
  - 展示的对话：**滚动 2 天窗口**（今天+昨天，每天 0 点清前天）+ **硬上限 ~1000 条**（超出批删最旧 ~500）；带时间戳；到顶提示"更早已清理"。
  - LLM 工作上下文：只喂最近几轮（多轮连贯），**不喂整 2 天**——省 token、浅环。
...
- **MVP 试用主走 weapp 体验版**（白名单测试者）。**H5 子域仅供团队内部开发 / 自动化测试**——按 [deploy/RUNBOOK.md](../../deploy/RUNBOOK.md)、[DEPLOYMENT.md](../../DEPLOYMENT.md)，**未备案的 HTTPS 域名只限内部验证、不得对外提供服务**，故**不拿来给桃子公开试用**。桃子公开用 H5、或正式上线，都需先 ICP 备案（PRD §10）。
