...
| `order_items` | `order`、`offering`、**`mealOccasion?`(同 occasion 枚举)**、`quantity`、`unitPriceCents?`(空=派生默认价)、`note?` | 跨午晚 = 同 order 多 item；桃子常 2 item；`mealOccasion` 经 LLM 智能分后**确认订单前必填**(occasion 粒度商家，**含 all-day：all-day 商家填 `occasion=all-day`、不可留空**，否则确认时按 (date,null) 撞不上当天 `occasion=all-day` 桶)；time-slot 商家留空、按 `startAt/endAt` 命中 slot；slot 归属 = occasion 粒度按 (order.date, mealOccasion)、time-slot 按 (order.date, 时段) 逻辑命中 service_slots 唯一键，不另设 FK |
