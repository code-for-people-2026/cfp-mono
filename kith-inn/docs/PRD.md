...
| 集合 | 关键字段 | 说明 |
|---|---|---|
| `customers` | `displayName`(识别键，不唯一)、`kind`(regular/walk-in/self)、`defaultAddress`(→customer_addresses 单值外键)、`defaultServings?`、`defaultOccasion?`(同 occasion 枚举)、`note?` | 接龙名字为识别；自家=kind:self（履约 onsite、不进送餐分拣，但份数/采购照算）；不存电话 |
...
| `service_slots` | `date`、`granularity`(occasion/time-slot)、**`occasion?`(枚举 breakfast/brunch/lunch/dinner/all-day；granularity=occasion 时必填、time-slot 时留空)**、`startAt/endAt?`(时段)、`status`(draft/open/archived)、`capacity?` | **双职责：通用时间桶（所有生意都有）+ 餐次/菜单锚（仅餐类）**。连续生意（奶茶=随时来单）= `granularity=occasion` + `occasion=all-day`，退化为「每天一个桶」、不存在"无 slot"——分拣/缺口/归档全按它成批、一套逻辑；`all-day` 作 occasion 值（非 null）是为让既有唯一约束 (seller,date,occasion) 原生生效、免手写 partial unique。slot 不预设"是餐"，餐次语义（午/晚、绑 menu_plan）由 menu-planning 模块叠加。**确认即开餐**：「确认订单」时 be 按该键 upsert slot→open（仅当 slot 缺失 / draft / open；`archived` 不被自动重开，需 force 二次确认），菜单发布同样开餐；draft / 取消都不碰 slot |
...
| `fulfillments` | **`orderItem`(挂餐次粒度)**、**`serviceDate`(index,反范式)**、**`occasion?`(反范式)**、`mode`(delivery/pickup/onsite)、`status`(pending/handed-off/done/**canceled**)、`addrBuilding`(index)、`addrUnit`、`assignee?`(受控值)、`sequence?`、`timeWindow?` | 履约薄表，**确认订单时建**（draft 不建；订单取消则置 `canceled` 终态、退出送餐口径，不真删——视图不回连 order，故取消必须在 fulfillment 自身可见）；地址定格快照、用餐日 + 餐次**随 order 实时同步**（改餐次/改日经 hook 回写，非冻结）；送餐视图按 (serviceDate,occasion,addrBuilding) 整栋成批，免回连 order_item→order |
...
- **delivery → 无自有表**：纯行为——`fulfillments` 上的 assignee/status/sequence/addrBuilding + 派生视图（分拣/缺口对账）+ tab/工具。"奶奶分片" = `assignee` 取值（来自 `moduleSettings.delivery.deliverers`），是**数据不是 schema**；单干商家不用 assignee。**status 收口**：`pending`(未处理) → `handed-off`(已交送餐人/奶奶、未到顾客，仅分片时用) → `done`(已送达＝语音勾销)；桃子自送则 pending→done 直达；订单取消则置 `canceled`(终态、退出送餐口径)。**缺口对账 = 数 `status ∈ {pending, handed-off}`**（未送达且未取消——不能只判 `≠done`，否则把已取消单误算成缺口）。
...
- **booking → `subscriptions`（V1）**：`customer`、`offering`、**`pattern`(json，必填 = source of truth：定义本订阅在哪些 (date,occasion) 上生成 order)**、`status`(active/paused,index)、`pausedRanges?`。**已删 `cadence`(weekly/monthly/adhoc) 枚举——"周/月"过死，真实是任意 (date,occasion) 集合，任何周期标签都能从 pattern 读时派生。** pattern 三类 shape：`specific-dates`(锁一串 date+occasion，最贴"预付某几天")／`recurring`(RRule 式如每周一三五，"一周一订"是其特例)／`open-ended`(长期默认直到取消)。订阅是 order 生成器（按租户物化、带 seller token、bypassPublicClose；禁用 admin key）。
