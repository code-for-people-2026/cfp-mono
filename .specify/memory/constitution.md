...
规格和任务必须拆成可独立验证、可独立 review 的 PR 切片；每个 PR 只承载一个目标或
核心不变量，并写明依赖顺序和独立验证方式。user story 是产品需求与最终验收单位，PR slice
...
每个功能计划必须列出最小但相关的自动化检查。Task ID 只承载代码/文档产物或可独立验收的
证据；通用的 diff 检查、仓库验证、开 PR、CI、review、resolve 和 merge 作为每个 PR 的统一
完成清单，不为每片重复制造功能任务。真机 smoke 等独立人工证据仍保留 Task ID，且不得用
自动化结果冒充完成。PR 必须遵守 `AGENTS.md`，并通过 `pr-review-converge` skill 收敛。
支持 skills 的 agent 必须使用该 skill；不支持 skills 的 agent 必须执行 `AGENTS.md` 中的等价
硬门禁。
...
- `plan.md` 和 `tasks.md` 必须包含按依赖排序的 PR 拆分计划；每片写明单一目标、主要路径和
  独立验证，且合并前保持门禁通过。
