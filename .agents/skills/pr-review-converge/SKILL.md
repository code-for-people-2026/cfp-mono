...
4. 从 plan/tasks 的 PR mapping 中选择依赖已经合并的下一片。先确认每个可执行 Task ID 恰好
   映射到一个 PR，且每片具备依赖和独立验证；缺失、重复或与 plan 冲突时先修复规划，不得
   把 story phase 直接当成一个 PR。不开 spec 的改动以 issue、用户任务或开始实现前的任务
   说明作为 PR 切片来源。每片开始前确认：
   - 一句话目标或核心不变量；
   - 精确允许路径和明确非目标；
   - 独立验证；
   - 依赖和人工 diff 预算。
...
循环，不得合并。不得 squash 或创建 merge commit。除非已经确认没有堆叠 PR 依赖该分支，
否则不要顺手 `--delete-branch`。
