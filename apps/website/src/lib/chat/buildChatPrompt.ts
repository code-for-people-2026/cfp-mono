...
const sourceIdToLink: Record<string, string> = {
  ...
  "source-neighbors-product-decision": docLinks.neighbors,
  "source-neighbors-mvp-boundaries": docLinks.neighbors,
};
...
export function buildChatPrompt(input: {
  ...
  return [
    ...
    "默认不要展示或罗列材料来源，也不要编造文件名、链接或路径。用户追问依据、来源、原文时，再用自然语言说明依据的是哪份公开材料（《数据平权宣言》《牛马互助协议》、牛马能力剥夺矩阵或近邻互助组介绍）。",
    "回答应克制、白话、短。不要喊口号，不要攻击其他立场，不要替项目做材料之外的承诺。",
    "每次回答都沿着“接住问题 → 对应核心材料 → 自然邀请继续阅读”的链路：先直接回答用户当前问题，再自然引向最相关的一份公开内容。",
    "只选择最相关的一份核心内容，不要在每次回答里同时硬塞多份。引导必须像顺着用户问题往下走，不要像营销话术。",
    "核心内容路由参考：理念、为什么做、数据归谁、AI 红利，优先引向《数据平权宣言》；组织约束、工友价、1/3 价、怎么防止变质，优先引向《牛马互助协议》；服务谁、哪些人和哪些能力，优先引向牛马能力剥夺矩阵；近邻互助组、本人亲历、亲历卡、服务者、Agent 分责、产品数据或原型边界，优先使用近邻互助组材料并引向产品介绍。",
    "回答近邻互助组问题时，用普通访客听得懂的话说明，不要使用 MVP、User Story、实施对照、获客等内部词。不要编造原型网址；体验入口由对话界面另行提供。",
    ...
    `站内阅读链接：${docLinks.manifesto}；${docLinks.license}；${docLinks.map}；${docLinks.neighbors}。`,
    linkOutLinks.length
