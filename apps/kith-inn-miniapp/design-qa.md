...
| 原问题 | 修正及最终证据 |
| --- | --- |
| P1 普通菜位卡片额外堆叠操作 | 卡片只保留换一道/自己选；做不做汤只在保存菜单的“调整某一餐”页面决定，整周汤位不再显示去汤/恢复。复制使用首页独立入口，周设置、保存草稿及放弃调整也在首页操作，编辑底部仅确认N餐进入review |
...
- 编辑/检查为七天十四餐，日期结束在周日；原型为五天十餐。三项导航继续使用街坊味菜品池命名，没有“我的”。做不做汤已移到独立单餐页，整周编辑各菜位均无去汤开关；已不做汤的占位明确显示“本餐不做汤”。
...
本周已保存菜单 → 调整某一餐 → 日期/午晚餐 → 本餐做汤开关 → 保存本餐调整。默认选中中国日期的今天且已启用的餐，否则选本周首个启用餐。开关只修改草稿，保存成功后仍留在该餐，可直接复制该餐已保存文字。整周修改页只改菜品，不再承担做不做汤的决定。

![周三晚餐不做汤，保存后仍留在单餐页](docs/visual-qa/single-meal-saved.png)

- 393×852 浏览器交互截图；周三晚餐、荤素菜、本餐做汤未选中、保存成功及复制入口同时可见。控件复用既有日期/餐次选择、卡片和底部按钮；不改变工程原型其他页面布局。H5复选框使用原生可访问名称，小程序使用原生Switch。
- 单餐必须从新读取的保存快照启动；未保存菜单、整周/设置草稿不能进入。切餐/退出遇到本地调整需明确放弃，取消不写入。停餐从选择器排除，无汤/全周停餐不能做无效修改。
- 保存以保存快照拼装，只允许目标餐 soupOmitted/必要恢复汤列表改变；confirm=false、rebuild=false，使用原周版本/幂等机制。保存后清原确认状态，不生成或重新确认整周。
- 浏览器用例断言全部14餐及周结构无旁路修改、保存前0新增写入、复制无汤、重新进入回看和恢复；覆盖失效多汤选齐、延迟读取时取消不可用、同页成功后再次调整清成功提示、409保留草稿、未知写入原键原正文重试，以及旧整周重排失败/复制保护。
- 截图及上述交互来自受控API浏览器测试，不冒充用户数据或微信真机证据。用户视觉确认和微信真机开关/剪贴板仍待验收。

验证记录：34条H5 E2E通过（`/tmp/single-meal-e2e-guards.log`）；追加已不做汤占位断言定向通过（`/tmp/single-meal-placeholder.log`），最终控件截图链路通过（`/tmp/single-meal-screenshot.log`）。根 `pnpm verify` 9/9通过，含H5/WeApp构建（`/tmp/single-meal-verify.log`）。
