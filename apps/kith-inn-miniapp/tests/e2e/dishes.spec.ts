...
async function openPool(page: Page, initial: Dish[] = []) {
  const state = { items: [...initial], writes: [] as { method: string; body: string; key: string }[], failure: "" };
  ...
  await page.route("**/api/kith-inn/dishes**", async (route) => {
    ...
    if (method === "GET") return route.fulfill({ json: { items: state.items }, headers });
  ...
  await expect(button(page, "批量添加")).toBeEnabled();
  await expect(button(page, "批量添加")).toHaveJSProperty("tagName", "BUTTON");
  return state;
}
async function preview(page: Page, names: string) {
  await button(page, "批量添加").focus();
  await expect(button(page, "批量添加")).toBeFocused();
  await page.keyboard.press("Enter");
  await page.locator('textarea[placeholder="每行一道菜，例如：红烧排骨"]').fill(names);
  await button(page, "预览分类").click();
}
