...
async function selectOrderCards(page: Page, payment: "到账○" | "到账✓", count = 3) {
...
test("E2E-ORDER-001 / E2E-MAIN-001：H5 从接龙连续完成菜单、到账记录与送达", async ({ page, request }) => {
  ...
  await expect(page.getByText("到账○", { exact: true })).toHaveCount(3);
  ...
  await selectOrderCards(page, "到账○");
  ...
  await page.locator("taro-button-core").filter({ hasText: /^批量标到账\(3\)$/ }).click();
  ...
  await selectOrderCards(page, "到账✓", 2);
  ...
  await expect(page.getByText("草稿 0 · 未标到账 0 · 待送 0", { exact: true })).toBeVisible();
