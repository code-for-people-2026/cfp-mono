...
test("顾客修改取消自有订单，桃子确认后锁单且停用资料不影响历史", async ({ page, request }) => {
  ...
  const batchProfileNames = [`备用资料甲-${suffix}`, `备用资料乙-${suffix}`];
  for (const displayName of batchProfileNames) expect((await request.post(`${BE}/customer/reservations`, {
    headers: customerHeaders, data: {
      batchPublicId: doc.publicId,
      profile: { newProfile: { displayName, address: "5B" } },
      displayName,
      address: "5B",
      items: [{ target: { date: dates[0], occasion: "lunch" }, quantity: 1, resubmitCanceled: false }]
    }
  })).ok()).toBe(true);

  await page.goto(share.path);
  let releaseOrders: () => void = () => undefined;
  const ordersReady = new Promise<void>((resolve) => { releaseOrders = resolve; });
  await page.route("**/customer/orders", async (route) => {
    await ordersReady;
    await route.continue();
  });
  await page.getByText("查看我的预订", { exact: true }).click();
  const copyDataButton = page.getByText("复制我的数据", { exact: true });
  await expect(copyDataButton).toHaveAttribute("disabled", "");
  await expect(page.locator(".page-state")).toContainText("正在加载我的预订…");
  releaseOrders();
  await expect(copyDataButton).not.toHaveAttribute("disabled", "");
  await page.unroute("**/customer/orders");
  const currentCards = page.locator(".customer-order-card").filter({ hasText: profileName });
  await page.evaluate(() => localStorage.removeItem("kith_inn_v1_customer_session"));
  await page.reload();
  await expect(currentCards).toHaveCount(2);
  await expect(page.locator(".customer-orders-page")).not.toContainText(PUBLIC_RISK_COPY);
  ...
  await expect(page.getByText(`确认停用${profileName}`, { exact: true })).toHaveCount(0);
  await expect(currentCards).toHaveCount(2);
  await page.context().grantPermissions(["clipboard-read", "clipboard-write"]);
  await page.getByText("复制我的数据", { exact: true }).click();
  await expect(page.getByText("我的数据已复制", { exact: true })).toBeVisible();
  const exportedText = await page.evaluate(() => navigator.clipboard.readText());
  const exported = JSON.parse(exportedText) as {
    schemaVersion: number;
    exportedAt: string;
    profiles: Array<{ displayName: string }>;
    orders: Array<{ displayName: string }>;
  };
  expect(exported.schemaVersion).toBe(1);
  expect(Number.isNaN(Date.parse(exported.exportedAt))).toBe(false);
  expect(exported.profiles.map(({ displayName }) => displayName)).not.toContain(profileName);
  expect(exported.profiles.map(({ displayName }) => displayName)).toEqual(expect.arrayContaining(batchProfileNames));
  expect(exported.orders.map(({ displayName }) => displayName)).toContain(profileName);
  expect(exportedText).not.toMatch(/"(?:sellerId|openid|token|operator|note|orderDeadline|orderStatus)"/);

  let deactivateRequests = 0;
  await page.route("**/customer/profiles/*/deactivate", async (route) => {
    deactivateRequests += 1;
    if (deactivateRequests === 2) return route.fulfill({
      status: 503,
      contentType: "application/json",
      body: JSON.stringify({ error: "temporary", message: "暂时失败" })
    });
    return route.continue();
  });
  await page.getByText("删除保存的资料", { exact: true }).click();
  await page.getByText(/^确认批量软停用 \d+ 条资料$/).click();
  await expect(page.getByText(/^已软停用 \d+ 条资料；1 条失败，可重试$/)).toBeVisible();
  const failure = page.locator(".deactivation-failure");
  await expect(failure).toHaveCount(1);
  const failedName = (await failure.textContent())?.replace("停用失败：", "").replace("，请重试", "") ?? "";
  await page.getByText(`停用资料：${failedName}`, { exact: true }).click();
  await page.getByText(`确认停用${failedName}`, { exact: true }).click();
  await expect(failure).toHaveCount(0);
  await expect(page.getByText("资料已软停用；历史预订仍可查看", { exact: true })).toBeVisible();
  await expect(page.getByText(/^停用资料：/)).toHaveCount(0);
  await expect(currentCards).toHaveCount(2);
  await page.getByText("查看个人信息用途说明", { exact: true }).click();
  await expect(page.getByText(
    "您可通过当前微信身份查看自己在当前商家下的常用资料和登记记录；当前商家可查看登记快照，用于确认和送达。",
    { exact: true }
  )).toBeVisible();
  await expect(page.getByText(
