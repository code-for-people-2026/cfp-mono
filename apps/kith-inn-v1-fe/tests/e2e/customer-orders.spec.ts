...
test("顾客修改取消自有订单，桃子确认后锁单且停用资料不影响历史", async ({ page, request }) => {
  ...
  await expect(page.locator(".deactivation-failure")).toHaveCount(1);
  await page.getByText("删除保存的资料", { exact: true }).click();
  await page.getByText("确认批量软停用 1 条资料", { exact: true }).click();
  await expect(page.getByText("已软停用 1 条资料；历史预订仍可查看", { exact: true })).toBeVisible();
