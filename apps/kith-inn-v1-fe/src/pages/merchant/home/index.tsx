...
import { buildMerchantMealCard, businessDateInShanghai, type MerchantMealCard } from "@/logic/merchantHome";
...
export default class MerchantHome extends Component<Record<string, never>, HomeState> {
  ...
  private load = async () => {
    ...
    this.setState({ date: today, status: "loading" });
    try {
      ...
      if (current === this.revision) this.setState({ meals: views, status: "loaded" });
    } catch (error) {
      if (current !== this.revision || error instanceof ApiError && (error.status === 401 || error.status === 403)) return;
    ...
    {status === "error" ? <View className="page-state error"><Text>今日数据加载失败</Text>
      ...
      <View className="home-meals">{meals.map(({ card, orderError }) => <View key={card.occasion} className="card home-meal-card">
        ...
        {card.slot ? <><Text className="home-menu">{card.slot.menuItems.map((item) => item.nameSnapshot).join(" · ")}</Text>
          ...
          <Button size="mini" onClick={() => go("/pages/merchant/menu/index")}>先排菜单</Button></>}
      </View>)}</View>
      <View className="home-quick">
        ...
        <Button onClick={() => go(deliveryUrl)}>配送清单</Button>
      </View>
    </>}
