...
export async function writeRelationshipsAvailable(payload: BasePayload, transactionReq: PayloadRequest, input: {
  sellerId: string | number; openid: string; batchPublicId: string; mealSlotId: string | number;
  customerProfileId: string | number;
}): Promise<NextResponse | null> {
  await lockCustomerAvailability(payload, transactionReq, input);
  const common = { limit: 1, depth: 0, overrideAccess: true, req: transactionReq } as const;
  const [batches, slots, profiles] = await Promise.all([
    payload.find({ collection: "kiv1_booking_batches", where: { and: [{ publicId: { equals: input.batchPublicId } },
      { seller: { equals: input.sellerId } }] }, ...common }),
    payload.find({ collection: "kiv1_meal_slots", where: { and: [{ id: { equals: input.mealSlotId } },
      { seller: { equals: input.sellerId } }] }, ...common }),
    payload.find({ collection: "kiv1_customer_profiles", where: { and: [{ id: { equals: input.customerProfileId } },
      { seller: { equals: input.sellerId } }, { openid: { equals: input.openid } },
      { active: { equals: true } }] }, ...common })
  ]);
  const batch = batches.docs[0] as { status?: unknown; mealSlots?: unknown } | undefined;
  const slot = slots.docs[0] as { orderStatus?: unknown; orderDeadline?: unknown } | undefined;
  if (!batch || !slot || profiles.docs.length === 0) {
    return NextResponse.json({ error: "relationship-owner-mismatch" }, { status: 409 });
  }
  if (!Array.isArray(batch.mealSlots) || !batch.mealSlots.some((id) => String(relationId(id)) === String(input.mealSlotId))) {
