import { KIV1_SERVICE_CLOSURE_CHECKED } from "@cfp/kith-inn-v1-payload";
  ...
  try {
    return await withKiv1Transaction(scope.payload, async (transactionReq) => {
      ...
      const closures = await scope.payload.find({
        ...
        where: { and: [
          ...
          ...(input.occasion === null ? [] : [{ or: [
            { occasion: { equals: null } },
            { occasion: { equals: input.occasion } }
          ] } as Where])
        ] },
        limit: 1,
        depth: 0,
      ...
      const openSlots = await scope.payload.find({
        ...
        where: { and: [
          { seller: { equals: scope.sellerId } },
          ...targetWhere(input.date, input.occasion),
      ...
      const doc = await scope.payload.create({
        ...
        context: { [KIV1_SERVICE_CLOSURE_CHECKED]: true },
        overrideAccess: true,
        req: transactionReq
      });
