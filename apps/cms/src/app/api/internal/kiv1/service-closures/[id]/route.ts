...
export async function DELETE(req: Request, { params }: RouteContext) {
  ...
  try {
    await withKiv1Transaction(scope.payload, async (transactionReq) => {
      ...
      const latest = await findOwned(
        scope.payload, "kiv1_service_closures", id, scope.sellerId, transactionReq
      );
      if (!latest) return;
    ...
    return NextResponse.json({ error: "service-closure-delete-failed" }, { status: 500 });
  }
}
