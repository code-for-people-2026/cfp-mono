...
import { bookingSettingsRoutes } from "./routes/bookingSettings";
...
export function assertV1ProductionEnv(env: Env = process.env): void {
  ...
  const previousJwt = env.KITH_INN_V1_PREVIOUS_JWT_SECRET?.trim();
  if (previousJwt && PLACEHOLDER.test(previousJwt)) {
    throw new Error("KITH_INN_V1_PREVIOUS_JWT_SECRET cannot be a placeholder");
  }
}
